...
func (vlr vlrHeader) FileSourceId() uint16 {
	return vlr.RecordId_
}

func (vlr vlrHeader) GlobalEncoding() uint16 {
	return vlr.RecordLength_
}

func (vlr vlrHeader) ProjectID1() [32]byte {
...
func (l *Lasf) readVlrs() (vlr,error) {
	vlrCount := int(l.h.VlrCount_)
	p.fin.Seek(int64(l.h.HeaderSize_), 0)
	var vlrh vlrHeader
	for i := 0; i < vlrCount; i++ {
		err := binary.Read(p.fin, binary.LittleEndian, &vlrh)
		...
		if strings.HasPrefix(userId, "LASF_Spec") {
			...
			case recordId = 4:
				...
				var extraBytes extraBytesDescriptor
				for i := 0; i < int(table.TableCount); i++ {
					err := binary.Read(l.fin, binary.LittleEndian, &extraBytes)
					if err != nil {
						return err
					}
				}
			...
			case 34736:
				fallthrough
			case 34737:
				p.pin.Seek(int64(vlrh.RecordLength_), 1)
				continue
			case 2111:
				fallthrough
			case 2112:
